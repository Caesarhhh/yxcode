...
from utils.metrics import polygon_area
import cv2
import numpy as np
import torch
...
from torch.utils.data import DataLoader, Dataset, dataloader, distributed,WeightedRandomSampler
...
def create_dataloader(path, imgsz, batch_size, stride, names, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0, ch=3, 
...
def create_dataloader_enhance(path, imgsz, batch_size, stride, names, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
                      rect=False, rank=-1, workers=8, image_weights=False, quad=False, prefix='', shuffle=False):
    if rect and shuffle:
        LOGGER.warning('WARNING: --rect is incompatible with DataLoader shuffle, setting shuffle=False')
        shuffle = False
    train_enhance_path="/mnt/cephfs/home/chenzhuokun/dataset/yx_datacode/synthesis_only_220412/train/images"
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(path, names, imgsz, batch_size,
                                      augment=augment,  # augmentation
                                      hyp=hyp,  # hyperparameters
                                      ...
                                      image_weights=image_weights,
                                      prefix=prefix)
        dataset_enhance = LoadImagesAndLabels(train_enhance_path, names, imgsz, batch_size,
                                      augment=augment,  # augmentation
                                      hyp=hyp,  # hyperparameters
                                      rect=rect,  # rectangular batches
                                      cache_images=cache,
                                      ...
                                      image_weights=image_weights,
                                      prefix=prefix)
    concat_datasets=ConcatDataset([dataset_enhance, dataset])
    concat_datasets.labels=dataset_enhance.labels+dataset.labels
    concat_datasets.shapes=np.vstack([dataset_enhance.shapes,dataset.shapes])
    batch_size = min(batch_size, len(dataset))
    nw = min([os.cpu_count() // WORLD_SIZE, batch_size if batch_size > 1 else 0, workers])  # number of workers
    #sampler = None if rank == -1 else distributed.DistributedSampler(concat_datasets, shuffle=shuffle)
    ratio=hyp['enhance_ratio']
    weights=torch.ones(len(concat_datasets))
    weights[0:len(dataset_enhance)]=ratio/(float(len(dataset_enhance))/len(dataset))
    sampler=WeightedRandomSampler(weights,len(concat_datasets),replacement=True)
    loader = DataLoader if image_weights else InfiniteDataLoader  # only DataLoader allows for attribute updates
    return loader(concat_datasets,
                  batch_size=batch_size,
                  shuffle=shuffle and sampler is None,
                  num_workers=nw,
                  sampler=sampler,
                  pin_memory=True,
                  collate_fn=LoadImagesAndLabels.collate_fn4 if quad else LoadImagesAndLabels.collate_fn), concat_datasets
...
class LoadImages:
    ...
    def __next__(self):
        ...
        #img = letterbox(img0, [int(img0.shape[0]/32)*32,int(img0.shape[1]/32)*32], stride=self.stride, auto=self.auto)[0]
        img = letterbox(img0, [self.img_size[0],self.img_size[1]], stride=self.stride, auto=self.auto)[0]
        # Convert
        if not self.gray:
...
def img2label_paths(img_paths):
    ...
    sa_coco, sb_coco = os.sep + 'images' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
    return [sb.join(x.rsplit(sa, 1)).rsplit('.', 1)[0] + '.txt' if x.find("coco")<0 else sb_coco.join(x.rsplit(sa_coco, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_names, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, ch=3, prefix=''):
        """
        Returns:
        ...
        self.gray = False if ch == 3 else True
        ...
        if self.rect:
            ...
            for i in range(nb):
                ...
                if maxi < 1: # batch图像高宽比均小于1时, shape=[h/w, 1] = [h_ratio, w_ratio]
                    ...
                    shapes[i] = [1, 1 / mini]
            if SLIDE:
                self.batch_shapes=self.shapes
                for i in range(self.batch_shapes.shape[0]):
                    if(min(self.batch_shapes[i][0],self.batch_shapes[i][1])<self.img_size):
                        self.batch_shapes[i][0]=int(self.batch_shapes[i][0]/32)*32
                        self.batch_shapes[i][1]=int(self.batch_shapes[i][1]/32)*32
            else:
                self.batch_shapes = np.ceil(np.array(shapes) * img_size / stride + pad).astype(np.int) * stride # (nb, [h_rect, w_rect])
                #self.batch_shapes[:,:]=self.img_size
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            #img_ada=cv2.adaptiveThreshold(img_gray,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,15,40)
            #img=img_ada
...
def Gaussin(im,ratio,kernel_size,sigma):
    if random.random()<ratio:
        im=cv2.cv2.GaussianBlur(im, (kernel_size,kernel_size), sigma)
    return im
...
def load_image_label(self, i):
    ...
    if im is None:  # not cached in ram
        ...
        #im=Gaussin(im,self.hyp["gaus_ratio"],self.hyp["gaus_kernel"],self.hyp["gaus_sigma"])
...
def load_mosaic(self, index):
    ...
    hbb_areas=(labels4[:,1::2].max(1)-labels4[:,1::2].min(1))*(labels4[:,2::2].max(1)-labels4[:,2::2].min(1))
    return img4, labels4[hbb_areas>=self.hyp["filter_small"],:]
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                l_ = []
                for label in labels:
                    if label[-1] == "0": # diffcult
                        continue
                    class_name=label[8]
                    if class_name=="LongDM":
                        class_name="DataMatrix"
                    cls_id = cls_name_list.index(class_name)
