...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    im = torch.zeros(batch_size, 1, *imgsz).to(device)  # image size(1,3,320,192) BCHW iDetection
    ...
    if ('onnx' in include) or ('openvino' in include):  # OpenVINO requires ONNX
        export_onnx(model, im, file, opset, train, dynamic, simplify,output=output)
...
def parse_opt():
    ...
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[288, 320], help='image (h, w)')
    ...
    parser.add_argument('--ch', type=int, default=3, help='TensorRT: workspace size (GB)')
