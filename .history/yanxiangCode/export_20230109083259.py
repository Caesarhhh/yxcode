...
from models.edgenext.layers import SDTAEncoderBNHS,ConvEncoderBNHS
import torch
...
def export_onnx(model, im, file, opset, train, dynamic, simplify, output="export/",prefix=colorstr('ONNX:')):
    # YOLOv5 ONNX export
    for k, m in model.named_modules():
        if isinstance(m, SDTAEncoderBNHS) and isinstance(m.act, nn.Hardswish):
            m.act = myHardswish()
        if isinstance(m, ConvEncoderBNHS) and isinstance(m.act, nn.Hardswish):
            m.act = myHardswish()
    ...
    try:
        ...
        if not os.path.exists(output):
            os.makedirs(output)
        #f=Path(os.path.join(output,file.parts[2]+".onnx"))
        f=Path(os.path.join(output,file.parts[2].split(".")[0]+".onnx"))
...
def export_saved_model(model, im, file, dynamic,
    ...
    try:
        ...
        im = tf.zeros((batch_size, *imgsz, 1))  # BHWC order for TensorFlow
        y = tf_model.predict(im, tf_nms, agnostic_nms, topk_per_class, topk_all, iou_thres, conf_thres)
        inputs = keras.Input(shape=(*imgsz, 1), batch_size=None if dynamic else batch_size)
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
        ...
        imgsz=(544, 640),  # image (height, width)
    ...
    if ('onnx' in include) or ('openvino' in include):  # OpenVINO requires ONNX
        export_onnx(model, im, file, opset, train, dynamic, simplify,output)
...
def parse_opt():
    ...
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[544, 640], help='image (h, w)')
    ...
    parser.add_argument('--conf-thres', type=float, default=0.1, help='TF.js NMS: confidence threshold')
    parser.add_argument('--output', type=str, default="export/", help='export path')
