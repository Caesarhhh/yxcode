...
from utils.datasets_origin import create_dataloader
from utils.general_origin import (LOGGER, box_iou, check_dataset, check_img_size, check_requirements, check_yaml,
                           coco80_to_coco91_class, colorstr, increment_path, non_max_suppression, print_args,
                           scale_coords, scale_polys, xywh2xyxy, xyxy2xywh, non_max_suppression_obb)
from utils.metrics import ConfusionMatrix, ap_per_class, ap_per_class_origin
...
def run(data,
    ...
    iouv = torch.linspace(0.8, 0.95, 10).to(device)  # iou vector for mAP@0.8:0.95
    if is_pcb:
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task], imgsz, batch_size, stride, names, single_cls, pad=pad, ch=ch, rect=pt,
    ...
    if len(stats) and stats[0].any():
        tp, fp, p, r, f1, ap, ap_class = ap_per_class_origin(*stats, plot=plots, save_dir=save_dir, names=names)
