...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80,anchors=(), ch=(), angle_num=180, use_gcn=False, anchor_type="ab",loss_type="normal",inplace=True):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5 + angle_num  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                #logits_.append(logits.view(bs, -1, self.no - 5-180))
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, MixConv2d, Focus, CrossConv, BottleneckCSP,
                ...
                GhostConv, ES_Bottleneck, ES_SEModule,Stem_EF,DWConvblock_ese,C2f,DWConv]:
            c1, c2 = ch[f], args[0]
            ...
            args.append(d.get('loss_type',"normal"))
        elif m is Contract:
            c2 = ch[f] * args[0] ** 2
