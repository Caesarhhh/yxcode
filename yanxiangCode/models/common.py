...
class DILATEConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False,dilation=2)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
...
class conv_bn_relu_maxpool_dilated(nn.Module):
    def __init__(self, c1, c2):  # ch_in, ch_out
        super(conv_bn_relu_maxpool_dilated, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(c1, c2, kernel_size=3, stride=2, padding=1, bias=False, dilation=2),
            nn.BatchNorm2d(c2),
            nn.ReLU(inplace=True),
        )
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
...
class DWConv_Shuffle(Conv):
    ...
    def forward(self, x):
        ...
        out = channel_shuffle(out, self.conv.groups)
        return out
...
class dwconv_bn_relu_maxpool(nn.Module):
    def __init__(self, c1, c2, g=1):  # ch_in, ch_out
        super(dwconv_bn_relu_maxpool, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(c1, c2, kernel_size=3, stride=2, padding=1, groups=g, bias=False),
            nn.BatchNorm2d(c2),
            nn.ReLU(inplace=True),
        )
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
...
class DWConvblock_ese(nn.Module):
    "Depthwise conv + Pointwise conv"

    def __init__(self, in_channels, out_channels, k, s):
        super(DWConvblock_ese, self).__init__()
        self.p = k // 2
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size=k, stride=s, padding=self.p, groups=in_channels,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(in_channels)
        self.eseattn=ESEAttn(in_channels)
        self.conv2 = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(out_channels)
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=False,n=1, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
...
class Shuffle_Block_ese(nn.Module):
    def __init__(self, inp, oup, stride,kernel_size=3,padding=1,dilation=1):
        super(Shuffle_Block_ese, self).__init__()
...
class Shuffle_Block_Dilated(nn.Module):
    def __init__(self, inp, oup, stride,kernel_size=3,padding=1,dilation=2):
        super(Shuffle_Block_Dilated, self).__init__()
        ...
        if self.stride > 1:
            self.branch1 = nn.Sequential(
                self.depthwise_conv_dialated(inp, inp, kernel_size=kernel_size, stride=self.stride, padding=padding,dilation=dilation),
                nn.BatchNorm2d(inp),
                nn.Conv2d(inp, branch_features, kernel_size=1, stride=1, padding=0, bias=False,dilation=dilation),
                nn.BatchNorm2d(branch_features),
                nn.ReLU(inplace=True),
            )
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(inp if (self.stride > 1) else branch_features,
                      branch_features, kernel_size=1, stride=1, padding=0, bias=False,dilation=2),
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
            self.depthwise_conv_dialated(branch_features, branch_features, kernel_size=kernel_size, stride=self.stride, padding=padding,dilation=dilation),
            nn.BatchNorm2d(branch_features),
            nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False,dilation=dilation),
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
        )

    @staticmethod
    def depthwise_conv_dialated(i, o, kernel_size, stride=1, padding=0, bias=False,dilation=2):
        return nn.Conv2d(i, o, kernel_size, stride, padding, bias=bias, groups=i,dilation=dilation)
...
class Shuffle_Block_Dilated_ese(nn.Module):
    def __init__(self, inp, oup, stride,kernel_size=3,padding=1,dilation=2):
        super(Shuffle_Block_Dilated_ese, self).__init__()
