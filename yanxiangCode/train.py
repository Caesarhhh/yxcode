...
import numpy as np
import torch
from utils.plots import plot_train_images
...
SLIDE=False
import val_origin  # for end-of-epoch mAP
import val
from models.experimental import attempt_load
from models.yolo import Model
from utils.tal_loss import TALLoss
from utils.autoanchor import check_anchors
from utils.autobatch import check_train_batch_size
from utils.callbacks import Callbacks
from utils.datasets import create_dataloader,create_dataloader_enhance
...
from utils.metrics import fitness, fitness_recall
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # val_path="/mnt/cephfs/home/chenzhuokun/dataset/yx_datacode/voc/test/images"
    nc = 1 if single_cls else int(data_dict['nc'])  # number of classes
    ch = data_dict.get('ch', 3) # input channels of images
    ...
    if hyp['enhance_ratio']>0:
        train_loader, dataset = create_dataloader_enhance(train_path, imgsz, batch_size // WORLD_SIZE, gs, names, single_cls,
                                                  hyp=hyp, augment=True, cache=opt.cache, rect=opt.rect, rank=LOCAL_RANK,
                                                  workers=workers, image_weights=opt.image_weights, quad=opt.quad,
                                                  prefix=colorstr('train: '), shuffle=True,ch=ch)
    else:
    ...
    if ch == 1 and hasattr(dataset, 'set_gray'):
        dataset.set_gray(True)
    ...
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, names, single_cls,
                                       ...
                                       prefix=colorstr('val: '),ch=ch)[0]
        typedict={}
        for index,name in enumerate(data_dict["names"]):
            typedict[name]=index
        val_gt_dict=val.getDictFromPath(data_dict["val"].replace("/images","/labelTxt"),typedict)
        if not resume:
    ...
    if cfg.find('down') >= 0:
        hyp['obj'] /= 2 * 2 # scale back to image size due to addtional down conv
    hyp['theta'] *= 3 / nl
    ...
    anchor_type=model.module.model[-1].anchor_type
    if hyp['loss_type']=='dfl':
        compute_loss=TALLoss(model)
    else:
        compute_loss = ComputeLoss(model,anchor_type=anchor_type)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        if opt.turn:
            #freeze_head_epochs=[range(0,40),range(200,240),range(400,440),range(600,800)]    
            #freeze_head_epochs=[y for x in [list(i) for i in freeze_head_epochs] for y in x]
            #minlr_epoch=600
            #if epoch==minlr_epoch:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #    plot_train_images(img,targets[targets[:,0]==index],"train_examples/train_{}.jpg".format(count))
            #    count+=1
            if len(imgs.shape)==3:
            ...
            with amp.autocast(enabled=cuda):
                if SLIDE:
                    loss=None
                    loss_items=None
                    preds = slide_model(model=model,im=imgs,imgsz=320,stride=200,augment=False,mode='train')
                    for index,pred in enumerate(preds):
                        if index<len(preds):
                            tar_index=(targets[:,2]>=pred[1][0]) * (targets[:,3]>=pred[1][1]) * (targets[:,2]<=pred[1][0]+320) * (targets[:,3]<=pred[1][1]+320)
                            tar=targets[tar_index].to(device).detach()
                            tar[:,2]-=pred[1][0]
                            tar[:,3]-=pred[1][1]
                            loss_, loss_items_ = compute_loss(pred[0], tar)
                        else:
                            tar=targets.to(device)
                            tar[:,2]*=320/imgs.shape[2]
                            tar[:,3]*=320/imgs.shape[3]
                            tar[:,4]*=320/imgs.shape[2]
                            tar[:,5]*=320/imgs.shape[3]
                            loss_, loss_items_ = compute_loss(pred[0],tar)
                        if index==0:
                            loss=loss_
                            loss_items=loss_items_
                        else:
                            loss=loss+loss_
                            loss_items=loss_items+loss_items_
                    loss=loss/len(preds)
                    loss_items=loss_items/len(preds)
                else:
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = val_origin.run(data_dict,
                                           ...
                                           use_obb=False,
                                           loss_type=hyp["loss_type"])


            # Update best mAP
            #fi = fitness(np.array(results).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            fi = fitness_recall(np.array(results).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95] with a larger weighted recall 
            if fi > best_fitness:
                best_fitness = fi
    ...
    if RANK in [-1, 0]:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = val_origin.run(data_dict,
                                            batch_size=batch_size // WORLD_SIZE * 2,
                                            imgsz=imgsz,
