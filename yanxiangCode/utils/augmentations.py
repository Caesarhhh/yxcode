...
def augment_hsv(im, hgain=0.5, sgain=0.5, vgain=0.5):
    ...
    brightness=ImageStat.Stat(Image.fromarray(im.astype(np.uint8)).convert('L')).rms[0]
    if hgain or sgain or vgain:
        if brightness < 22:
            r = np.random.uniform(0, 1, 3) * [hgain, sgain, vgain] + 1  # random gains
        elif brightness > 227:
            r = np.random.uniform(-1, 0, 3) * [hgain, sgain, vgain] + 1  # random gains
        else:
            r = np.random.uniform(-1, 1, 3) * [hgain, sgain, vgain] + 1  # random gains
        hue, sat, val = cv2.split(cv2.cvtColor(im.astype(np.uint8), cv2.COLOR_BGR2HSV))
        ...
        cv2.cvtColor(im_hsv.astype(np.uint8), cv2.COLOR_HSV2BGR, dst=im)  # no return needed
