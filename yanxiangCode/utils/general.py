...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=1500,loss_type="csl"):
    ...
    t = time.time()
    output = [torch.zeros((0, 7), device="cuda")] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
...
def non_max_suppression_obb_distill(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=1500,loss_type="csl",distill=False):
    """Runs Non-Maximum Suppression (NMS) on inference results_obb
    Args:
        prediction (tensor): (b, n_all_anchors, [cx cy l s obj num_cls theta_cls])
        agnostic (bool): True = NMS will be applied between elements of different categories
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if distill:
            ...
            logits = x[:, 5:class_index].sigmoid() * x[:, 4:5]
        else:
            # x[:, 5:] *= x[:, 4:5]
            logits = x[:, 5:class_index] * x[:, 4:5]
        #x[:, 5:class_index] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            ...
            x = torch.cat((x[i, :4], theta_pred[i], x[i, j + 5, None], logits), 1)
        else:  # best class only
            conf, j = logits.max(1, keepdim=True)
            x = torch.cat((x[:, :4], theta_pred, conf, logits), 1)[conf.view(-1) > conf_thres]
