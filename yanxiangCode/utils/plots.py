...
def plot_train_images(image,targets,fname):
    image_draw=image.clone().permute(1,2,0).cpu().numpy().astype(np.uint8)
    for target in targets:
        image_draw=cv2.rectangle(image_draw.copy(),(int(target[2]-target[4]*0.5),int(target[3]-target[5]*0.5)),(int(target[2]+target[4]*0.5),int(target[3]+target[5]*0.5)),(0,255,0),2)
    cv2.imwrite(fname,image_draw)
