...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    #bpr=0.95
    #anchors_origin=anchors.cpu().numpy().reshape((9,2))
    if bpr > 0.98:  # threshold to recompute
        ...
        try:
            anchors = kmean_anchors(dataset, n=anchors.shape[0]*anchors.shape[1], img_size=imgsz, thr=thr, gen=1000, verbose=False)
            #anchors=np.vstack((anchors_origin[0:3],anchors[3:9]))
