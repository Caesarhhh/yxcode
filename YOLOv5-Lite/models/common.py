...
class Shuffle_Block_Dilated(nn.Module):
    def __init__(self, inp, oup, stride):
        ...
        if self.stride > 1:
            self.branch1 = nn.Sequential(
                self.depthwise_conv_dialated(inp, inp, kernel_size=3, stride=self.stride, padding=2,dilation=2),
                nn.BatchNorm2d(inp),
                nn.Conv2d(inp, branch_features, kernel_size=1, stride=1, padding=0, bias=False,dilation=2),
                nn.BatchNorm2d(branch_features),
                nn.ReLU(inplace=True),
            )
        ...
        self.branch2 = nn.Sequential(
            ...
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
            self.depthwise_conv_dialated(branch_features, branch_features, kernel_size=3, stride=self.stride, padding=2,dilation=2),
            nn.BatchNorm2d(branch_features),
            nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False,dilation=2),
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
        )
