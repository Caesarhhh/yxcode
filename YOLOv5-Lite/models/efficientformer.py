...
class Stem_EF(nn.Module):
    def __init__(self,in_chs, out_chs):
        super().__init__()
        self.block=stem(in_chs,out_chs)

    def forward(self,x):
        return self.block(x)

class Embedding_EF(nn.Module):
    """
...
class Meta3D(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.permute(0,2,1).view(-1,x.shape[2],int(math.sqrt(x.shape[1])),int(math.sqrt(x.shape[1])))
...
class MetaBlocks(nn.Module):
    def __init__(self,dim, index, layers,vit_num=1,
                pool_size=3, mlp_ratio=4.,
                act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                drop_rate=.0, drop_path_rate=0.,
                use_layer_scale=True, layer_scale_init_value=1e-5):
        super().__init__()
        self.blocks=meta_blocks(dim, index, layers,
                pool_size, mlp_ratio,
                act_layer, norm_layer,
                drop_rate, drop_path_rate,
                use_layer_scale, layer_scale_init_value, vit_num)
...
class EfficientFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
                 downsamples=None,vit_num=0,
                 mlp_ratios=4, pool_size=3,
