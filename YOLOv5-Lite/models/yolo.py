...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect) or isinstance(m, ETDetect):
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR, Shuffle_Block,Shuffle_Block_Dilated,Shuffle_Block_ese, conv_bn_relu_maxpool,conv_bn_relu_maxpool_dilated,DWConvblock, MBConvBlock, LC3, 
                 RepVGGBlock, SEBlock, mobilev3_bneck, Hswish, SELayer, stem, CBH, LC_Block, Dense,
                GhostConv, ES_Bottleneck, ES_SEModule,DWConvblock_ese,Stem_EF,C2f]:
            c1, c2 = ch[f], args[0]
